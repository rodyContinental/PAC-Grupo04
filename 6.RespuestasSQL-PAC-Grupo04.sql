...
V.nombre, V.apellido, SUM(DP.subtotal) AS TotalFacturacion, CONVERT(DECIMAL(10,2), (SUM(DP.subtotal) / TS.Total) * 100) AS PorcentagePedidos
FROM grupo04.VENDEDOR V
JOIN grupo04.PEDIDO P ON V.id_vendedor = P.id_vendedor
JOIN grupo04.DETALLE_PEDIDO DP ON P.id_pedido = DP.id_pedido
CROSS JOIN TotalPedidos TS
WHERE YEAR(P.fecha_pedido) = 2023 AND DATEPART(QUARTER,P.fecha_pedido) = 4
...
JOIN grupo04.PEDIDO P ON V.id_vendedor = P.id_vendedor 
JOIN grupo04.DETALLE_PEDIDO DP ON P.id_pedido = DP.id_pedido 
WHERE YEAR(P.fecha_pedido) = 2023 AND DATEPART(QUARTER,P.fecha_pedido) = 3 
...
CONVERT(DECIMAL(10,2), ((Q4.TotalQ4 - Q3.TotalQ3) / Q3.TotalQ3) * 100) AS IncrementoPorcentual 
FROM grupo04.VENDEDOR V 
LEFT JOIN Q3Pedidos Q3 ON V.id_vendedor = Q3.id_vendedor 
LEFT JOIN Q4Pedidos Q4 ON V.id_vendedor = Q4.id_vendedor 
ORDER BY IncrementoPorcentual DESC;
...
SELECT TOP 3 
PR.nombre_producto, 
SUM(DP.subtotal) AS TotalVentasProducto, 
CONVERT(DECIMAL(10,2), (SUM(DP.subtotal) / TF.Total) * 100) AS PorcentajeFacturacion 
FROM grupo04.DETALLE_PEDIDO DP 
JOIN grupo04.PEDIDO P ON DP.id_pedido = P.id_pedido 
JOIN grupo04.PRODUCTO PR ON DP.id_producto = PR.id_producto 
CROSS JOIN TotalFacturacion TF 
WHERE YEAR(P.fecha_pedido) = 2023 AND DATEPART(QUARTER,P.fecha_pedido) = 4 
GROUP BY PR.nombre_producto, TF.Total 
ORDER BY SUM(DP.subtotal) DESC;
...
JOIN grupo04.DETALLE_PEDIDO DP ON P.id_pedido = DP.id_pedido
WHERE YEAR(fecha_pedido) = 2023 AND DATEPART(QUARTER,P.fecha_pedido) BETWEEN 1 AND 2
GROUP BY id_cliente
),
ClienteS2 AS (
SELECT 
...
FROM grupo04.PEDIDO P
JOIN grupo04.DETALLE_PEDIDO DP ON P.id_pedido = DP.id_pedido
WHERE YEAR(fecha_pedido) = 2023 AND DATEPART(QUARTER,P.fecha_pedido) BETWEEN 3 AND 4
GROUP BY id_cliente
)
SELECT TOP 3
C.id_cliente, 
...
S1.TotalS1,
S2.TotalS2,
((S2.TotalS2 - S1.TotalS1) * 100 / S1.TotalS1) AS [% Incremento]
FROM ClienteS1 S1
